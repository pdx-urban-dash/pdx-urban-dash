...
// TODO: grab all the sheets into an array
...
class GoogleSheetsAPI extends RESTDataSource {
  ...
  valueProcessor(data) {
    ...
    chart.dataSets = this.dataSetProcessor(data.values.slice(14), chart.type);
    return chart;
  }
    
  dataSetProcessor(chart, type) {
  ...
  async getCharts() {
    ...
    const ranges = titles.map(chart => `${chart.title}!A1:${this.toColumn(chart.gridProperties.columnCount)}${chart.gridProperties.rowCount}`);
    const request = {
      spreadSheetID: sheetID,
      auth: this.genAuth(),
      ranges,
    ...
    const results = await retrieveData;
    return results.map(this.valueProcessor);
    //   return this.get(`${sheetID}/values/`, param).then();
  }
}
